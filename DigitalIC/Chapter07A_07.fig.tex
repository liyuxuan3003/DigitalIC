...
\path (xPa/yB) node[not port] (I0) {} node[above=0.25cm] {$I_1$}; 
...
\path (xPb/yB) node[pmos,rotate=-90] (T0p) {};

\path (xPb/yA) node[nmos,rotate=90] (T1n) {} node {$T_{0}$};
\path (xPb/yA) node[pmos,rotate=-90] (T1p) {};
...
\draw (I2.out) -- (xR/yC) node[ocirc] {} node[right] {$Q$};
\draw (xQ/yC) node[circ] {} -- (xQ/yI) -- (xL/yI) -- (xL/yA) -- (I1.in);
\draw (I0.in) -- (xL |- I0.in) node[ocirc] {} node[left] {$D$};

\draw (T1n.G) -- (T0p.G);
\draw (xPb/yC)  -- (xPbx/yC) node[ocirc] {} node[left] {$\xbar{\te{CLK}}$};
\draw (T1p.G) node[ocirc] {} node[above] {$\te{CLK}$};
\draw (T0n.G) node[ocirc] {} node[below] {$\te{CLK}\vphantom{\CLK*}$};
