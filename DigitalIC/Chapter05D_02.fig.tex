...
\draw
(xCL/yC) node[above] {\small $0\xlarr[$t_\textit{pLH}$]V_{DD}/2\xlarr V_{DD}$}
to[C=$C_L$,*-]
(xCL/yB) node[ground] {};

\draw[ultra thin] (xBorL/yBorB) rectangle (xBorR/yBorA);

\path (xC/yPMOSa) node[right] {\tiny PMOS速度饱和};
\path (xC/yNMOSa) node[right] {\tiny NMOS截止};
