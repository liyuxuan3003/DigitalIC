...
\draw (pmos.D|-yC) to[short,i_={\small $I_{Dp0}$}] (pmos.D);
\draw (nmos.D|-yC) to[short,i^={\small $I_{Dn0}$}] (nmos.D);
...
\path (pmos.S) node[left] {\small $V_{GSp}$};
\path (nmos.S) node[left] {\small $V_{GSn}$};
\path (pmos) node[right=0.4cm] {\small $V_{DSp}$};
\path (nmos) node[right=0.4cm] {\small $V_{DSn}$};
