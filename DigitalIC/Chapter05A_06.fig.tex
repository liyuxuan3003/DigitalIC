...
\draw (pmos.D|-yC) to[short,i_<={\small $I_{Dp}$}] (pmos.D);
\draw (nmos.D|-yC) to[short,i^={\small $I_{Dn}$}] (nmos.D);
...
\path (pmos.S) node[left] {\small $V_{in}-V_{DD}$};
\path (nmos.S) node[left] {\small $V_{in}$};
\path (pmos) node[right=0.4cm] {\small $V_{out}-V_{DD}$};
\path (nmos) node[right=0.4cm] {\small $V_{out}$};
